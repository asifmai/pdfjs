	...
	for (var field_key in field_specs) {
		...
		field_specs[field_key].forEach(function (spec, i) {
			...
			if (spec.type === 'boolean') {
				...
				if (fillData[input.getAttribute('data-key')]) {
					input.checked = fillData[input.getAttribute('data-key')]
				}
			} else if (spec.type === 'string') {
				if (fillData[input.getAttribute('data-key')]) {
					input.setAttribute('value', fillData[input.getAttribute('data-key')]);
				}
